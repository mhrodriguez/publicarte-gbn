   ...
   <div class="content accounting-template">
      <div class="row">
         <?php
            ...
            if(isset($proposal) || ($this->input->get('contact_id') && $this->input->get('client_id'))){
               ...
               <div class="panel-body">
                  <div class="row">
                     ...
                     <div class="col-md-6 border-right">
                        ...
                        <!-- FIN DE CONTROLES OCULTOS-->



                        <div  class="form-group">
                           <label for="client_id" class="control-label"><?php echo "Cliente"; ?></label>
                           <select name="client_id" id="client_id" class="selectpicker" data-width="100%"  data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                              <option value=""></option>
                              <?php 
                                  foreach ($clientes as $cliente) {
                                   echo '<option value="'.$cliente['userid'].'" ' . ((isset($proposal) && $proposal->client_id==$cliente['userid'] || $this->input->get("client_id")) ? 'selectd' : '') .' >'.$cliente['company'].'</option>';
                                  }
                              ?>
                          </select>
                        </div>
                        <div class="form-group>" id="contact_id_wrapper">
                           <label for="contact_id"><?php echo "Atención a";?></label>
                           <div id="contact_id_select">
                              <select name="contact_id" id="contact_id" class="selectpicker" data-width="100%" data-live-search="true" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                <option value=''></option>
                              </select>
                           </div>
                        </div>
         ...
         <div class="col-md-12">
            <div class="panel_s">
               <?php $this->load->view('admin/estimates/my_add_edit_items_for_proposal'); ?>
            </div>
            <div class="panel_s">
               <div class="panel-body">
...
<script>
   ...
   _contact_id_wrapper =$('#contact_id_wrapper'),
  ...
  _client_id.on('change',function(){
    var value=$(this).val();
      /*clon = _contact_id.html('').clone();
      _contact_id.selectpicker('destroy').remove();
      _contact_id=clon;*/

    $.ajax({
      url: admin_url + "clients/get_contacts_of_customer",
      type: "post",
      data: {"customer_id" : value},
      success: function(data){
        _contact_id.html('<option value=""></option>');
        _contact_id.append(data);
        _contact_id.selectpicker('refresh');
      },
    });
